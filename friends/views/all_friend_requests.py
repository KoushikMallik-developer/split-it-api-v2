...
class AllFriendRequestsView(APIView):
    ...
    def get(self, request):
        try:
            ...
            all_friend_requests=UseFriendServices.get_all_friend_requests()
            if validate_user_uid(uid=user_id).is_validated:
                return Response(
                    data={"data":all_friend_requests.model_dump(), "errorMessage":None}, status=status.HTTP_200_OK,
            ...
            return Response(
                data={"successMessage":None, "errorMessage":f"NotImplementedError: {e}", },
                status=status.HTTP_500_INTERNAL_SERVER_ERROR, content_type="application/json", )
        except Exception as e:
            logging.error(f"InternalServerError: {e}")
            return Response(
                data={"successMessage":None, "errorMessage":f"InternalServerError {e}", },
                status=status.HTTP_500_INTERNAL_SERVER_ERROR, content_type="application/json", )
