    ...
    def post(self, request: Request):
        try:
            ...
            if validate_user_uid(uid=user_id).is_validated:
                result = SettlementService().add_settlement(
                    data=AddSettlementRequestType(**request.data)
                )
                return Response(
                    data={
                        "message": (
                            ...
                            else "Settlement balance added successfully"
                        ),
                        "data": result.get("settlement").model_dump(),
                    },
